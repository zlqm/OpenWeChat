_error_code_hint_list = [
    ...
    (40014, '不合法的 access_token ，请开发者认真比对 access_token 的有效性（如是否过期），'
     '或查看是否正在为恰当的公众号调用接口'),
    (40015, '不合法的菜单类型'),
    (40016, '不合法的按钮个数'),
    (40017, '不合法的按钮个数'),
    ...
    (41008, '缺少 oauth code'),
    (41009, '缺少 openid'),
    (42001, 'access_token 超时，请检查 access_token 的有效期，请参考基础支持 -'
     ' 获取 access_token 中，对 access_token 的详细机制说明'),
    ...
    (61454, '客服帐号名长度超过限制 ( 仅允许 10 个英文字符，不包括 @ '
     '及 @ 后的公众号的微信号 )(invalid   kf_acount length)'),
...
ERROR_CODE_HINT_MAPPPING = {key: value for key, value in _error_code_hint_list}
